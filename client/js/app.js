...
function renderNav() {
  ...
  header.innerHTML = `
    <nav class="nav">
      ...
      <div class="nav-right">
        ${user ? `<span class="nav-username">Signed in as</span>` : ''}
...
async function switchAccount(username) {
  ...
  if (res.ok && res.data.success) {
    ...
    // Re-render current page so auth-gated content loads
    route();
  } else {
...
async function ensureAuth() {
  ...
  // Auto sign-in as default test user
  ...
  if (!authed) {
    main.innerHTML = `
      <div class="empty-state">
        ...
        <div class="empty-state-title">Cannot connect to server</div>
        <p style="margin-top:8px">Make sure all 5 Spring Boot services are running, then refresh.</p>
      </div>`;
    return;
  }
